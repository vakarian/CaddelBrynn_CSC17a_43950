...
bool isClear(Settings *, short, short);
...
void playMS(short, short, Settings::Difficulty, char*);
    ...
    if (ans == 'y') {
        ...
        if (check(nrows, nrows, userSet)) {
            while (ans == 'y' && check(nrows, nrows, userSet)) {
                playMS(nrows, nrows, userSet, player);
                cout << endl;
...
void playMS(short nrows, short ncols, Settings::Difficulty dLevel, char *p) {
    ...
    //Once the game has wither been won or lost
    //Win Case
    if (hasWon(mineFld)) {
...
short proxM(Settings *mineFld, short row, short col, short FLAG) {
    ...
    /// top left corner
        ...
        if (mineFld->field[row][col+1] == Settings::MINE) ++nAd;
        if (mineFld->field[row+1][col] == Settings::MINE) ++nAd;
        if (mineFld->field[row+1][col+1] == Settings::MINE) ++nAd;
    }
    /// top right corner
...
/// selected square
bool isClear(Settings * mineFld, short row, short col) {
...
void showZeros(Settings *mineFld, short row, short col) {
    ...
    if ( row >= mineFld->rows || row < 0 || col >= mineFld->columns || col < 0)
        return;
    if (isClear(mineFld, row, col) && mineFld->field[row][col] != Settings::CLEAR){
...
void fields() {
    ...
    Settings **mineFld = new Settings*[n];
    const int row = 10;
    const int col = 10;
    /// create the fields
    for (int i = 0; i != n; ++i) {
        /// Create each field
        mineFld[i] = create(row, col);
        /// get number of mines
        mineFld[i]->mines = nMines(Settings::BEGINNER);
        /// set the mines
        setMines(*(mineFld+i));
        /// set the flags
        setFlags(*(mineFld+i));
